"""
services/receipt_service.py — Business logic for /extract and /validate (async).
...
Both functions:
  ...
  3. Call model.predict.predict_receipt() to get ML results.
     (predict_receipt is a sync/CPU-bound stub — called directly, no await)
...
async def extract_receipt(receipt_id: str) -> dict | None:
    """
    ...
    """
    meta = await get_receipt_metadata(receipt_id)
    if not meta:
        logger.warning("extract_receipt — receipt_id=%s not found", receipt_id)
        return None

    file_bytes = await get_receipt_bytes(receipt_id)
    ...
    await update_receipt_metadata(receipt_id, {"extracted": True, "extraction": result})
    logger.info("extract_receipt OK — receipt_id=%s", receipt_id)
    return result


async def validate_receipt(receipt_id: str) -> dict | None:
    """
    ...
    await update_receipt_metadata(receipt_id, {"validated": True, "validation": result})
